...
export default function SightseeingPage() {
  ...
  useEffect(() => {
    ...
    const loadApp = async () => {
      try {
        // Next.jsの動的インポートを使用
        const { generateStampRallyHTML, handleQueryParameters, initializeStampRally } = await import('./route_navigator.js');
        ...
        if (container) {
          ...
          setTimeout(() => {
            ...
            if (typeof window !== 'undefined') {
              // スタンプラリーを初期化
              initializeStampRally();
              
              const params = handleQueryParameters();
              
              if (params.autoStart) {
                // 直接スタンプラリーを開始
                if (typeof window.startStampRally === 'function') {
                  window.startStampRally();
                }
              } else if (params.showQR) {
                // QRモーダルを表示
                if (typeof window.showQrIntro === 'function') {
                  window.showQrIntro();
                }
                
                if (params.autoScan && typeof window.startQRScanning === 'function') {
                  // 自動的にQRスキャンを開始
                  setTimeout(() => {
